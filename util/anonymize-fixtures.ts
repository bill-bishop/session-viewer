...
import { readFileSync, writeFileSync, readdirSync, renameSync, statSync, mkdirSync, existsSync } from 'fs'
...
function nextLorem(count: number): string {
  const words: string[] = []
...
function loremForLength(original: string): string {
  ...
  if (result.length > targetLen + 20) return result.slice(0, targetLen)
  return result
}
...
function looksLikePath(value: string): boolean {
  ...
  if (/^C--Users-/.test(value)) return true
  // Relative paths with extensions
  if (/\.(ts|js|json|md|css|html|txt|yaml|yml|toml|py|go|rs|java|sh|bash)$/i.test(value)) return true
  return false
}
...
function scrambleValue(key: string, value: unknown): unknown {
  ...
  if (typeof value === 'object') {
    return scrambleObject(value as Record<string, unknown>)
  }
  ...
  if (PRESERVE_KEYS.has(key)) return str
  if (SLUG_KEYS.has(key)) return fakeSlug()
  if (PATH_KEYS.has(key)) return fakePath()
  if (TEXT_KEYS.has(key)) return loremForLength(str)

  // Heuristic: if it looks like a path, replace it
  if (looksLikePath(str)) return fakePath()

  // Unknown string keys: replace with lorem if longish, keep short identifiers
  if (str.length > 30) return loremForLength(str)
  return str
}

function scrambleObject(obj: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, value] of Object.entries(obj)) {
    if (PRESERVE_KEYS.has(key)) {
      ...
      result[key] = scrambleTrackedFileBackups(value as Record<string, unknown>)
    } else {
...
function scrambleMessage(msg: Record<string, unknown>): Record<string, unknown> {
  const result: Record<string, unknown> = {}
...
function scrambleContent(content: unknown): unknown {
...
function scrambleSnapshot(snapshot: Record<string, unknown>): Record<string, unknown> {
  const result: Record<string, unknown> = {}
...
function scrambleTrackedFileBackups(backups: Record<string, unknown>): Record<string, unknown> {
  const result: Record<string, unknown> = {}
...
function scrambleContentBlock(block: unknown): unknown {
...
function processMetaJson(filePath: string) {
  const content = readFileSync(filePath, 'utf-8')
  try {
